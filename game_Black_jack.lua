...
function drawDisplayForOneHand()
  ...
  for i = 0,4 do gpu.fill(9+i*5,6,4,4,' ') end
  for i = 0,4 do gpu.fill(9+i*5,15,4,4,' ') end
	...
	if (login) and p == player then
		if blackjack then
			...
			elseif (x>=22 and y==11 and x<33) then
				give_card_diller()
				if (count_cards(diller_cards,true)==21) then localsay("Black Jack!, победа казино!")
				else rewardPlayer(player,value*1.5,"Black Jack!") end
			end
